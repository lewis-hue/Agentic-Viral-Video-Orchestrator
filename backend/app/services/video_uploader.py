...
async def upload_instagram_reel(path: str, request) -> PlatformResult:
    """
    ...
    try:
        ...
        # In production, upload to a service like AWS S3 first
        video_url = await upload_to_public_host(path)
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def upload_facebook_video(path: str, request) -> PlatformResult:
    """
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def upload_tiktok_video(path: str, request) -> PlatformResult:
    """
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def upload_twitter_video(path: str, request) -> PlatformResult:
    """
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def upload_youtube_video(path: str, request) -> PlatformResult:
    """
...
async def upload_to_public_host(path: str) -> str:
    ...
    Supports Cloudinary or AWS S3 based on environment configuration
    """
    service = os.getenv("MEDIA_UPLOAD_SERVICE", "cloudinary").lower()

    if service == "cloudinary":
        return await upload_to_cloudinary(path)
    elif service == "s3":
        return await upload_to_s3(path)
    else:
        # Fallback to placeholder
        logger.warning("Unknown MEDIA_UPLOAD_SERVICE, using placeholder")
        return f"https://example.com/videos/{os.path.basename(path)}"

async def upload_to_cloudinary(path: str) -> str:
    ...
    try:
        ...
        # Upload file
        response = cloudinary.uploader.upload(path, resource_type="auto")
        return response['secure_url']

    except ImportError:
        logger.error("Cloudinary library not installed. Install with: pip install cloudinary")
        raise
    except Exception as e:
        logger.error(f"Error uploading to Cloudinary: {e}")
        raise

async def upload_to_s3(path: str) -> str:
...
async def upload_via_zapier(path: str, request) -> PlatformResult:
    """
    Upload video to Cloudinary and schedule via Zapier for all platforms
    """
    try:
        # Upload to Cloudinary first
        media_url = await upload_to_cloudinary(path)
        
        # Always use all three platforms for Zapier
        ...
        zapier_payload = {
            "video": {
                ...
                "description": request.description,
                "platforms": {
                    "tiktok": {"enabled": "tiktok" in platforms},
                    "instagram": {"enabled": "instagram" in platforms},
                    "youtube": {"enabled": "youtube" in platforms}
                }
            }
        }
        if hasattr(request, 'publish_at') and request.publish_at:
            zapier_payload["video"]["platforms"]["tiktok"]["schedule"] = {"datetime": request.publish_at.isoformat()}
            zapier_payload["video"]["platforms"]["instagram"]["schedule"] = {"datetime": request.publish_at.isoformat()}
            zapier_payload["video"]["platforms"]["youtube"]["schedule"] = {"datetime": request.publish_at.isoformat()}

        result = schedule_post_via_zapier(zapier_payload)
        ...
        if result.get("status") == "success":
            return PlatformResult(
                ...
                status="success",
                url=media_url,
                metadata={"zapier_response": result}
            )
        else:
            return PlatformResult(
                platform="zapier",
        ...
        logger.error(f"Error in upload_via_zapier: {e}")
        return PlatformResult(
            platform="zapier",
            status="error",
            error_message=str(e)
...
async def upload_to_platform(platform: str, path: str, request) -> PlatformResult:
    """
    Route upload request to appropriate platform function
    """
    if platform.lower() == "zapier":
        return await upload_via_zapier(path, request)
    ...
    return await upload_func(path, request)