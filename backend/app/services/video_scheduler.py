...
from .video_uploader import upload_to_public_host
...
def schedule_publish(
    ...
    if run_at is None and hasattr(request, 'publish_at') and request.publish_at:
        try:
...
async def execute_scheduled_publish(job_id: str) -> None:
    ...
    try:
        ...
        class PublishRequest(BaseModel):
            ...
            voice_to_text: Optional[bool] = False
            tags: List[str] = []
        ...
        try:
            media_url = await upload_to_public_host(job.video_path)
            logger.info(f"Uploaded video to public URL: {media_url}")
        except Exception as e:
            logger.error(f"Failed to upload video to public host: {e}")
            job.status = "failed"
            job.error_message = str(e)
            return

        # Schedule post via Zapier for all platforms
        try:
            logger.info(f"Scheduling post via Zapier for platforms: {request.platforms}")
            zapier_payload = {
                "video": {
                    "url": media_url,
                    "description": request.description,
                    "platforms": {
                        p: {"enabled": True, "schedule": {"datetime": job.scheduled_time.isoformat()}} for p in request.platforms
                    }
                }
            }
            ...
            results = {}
            if zapier_result["status"] == "success":
                # Assume success for all platforms since Zapier handles it
                for platform in request.platforms:
                    results[platform] = {
                        "platform": platform,
                        "status": "success",
                        "media_id": "via_zapier",
                        "metadata": zapier_result.get("response")
                    }
                logger.info(f"Successfully scheduled via Zapier: {zapier_result}")
            else:
                # Error for all platforms
                for platform in request.platforms:
                    results[platform] = {
                        "platform": platform,
                        "status": "error",
                        "error_message": zapier_result["error_message"]
                    }
                logger.error(f"Failed to schedule via Zapier: {zapier_result}")

        except Exception as e:
            logger.error(f"Exception in Zapier scheduling: {e}")
            results = {}
            for platform in request.platforms:
                results[platform] = {
                    "platform": platform,
